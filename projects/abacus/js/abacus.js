export class Abacus {
  ...
  bindEventHandlers() {
    ...
    this.container
      .querySelector("#lessBtn")
      ?.addEventListener("click", this.handleNumberButtonClick.bind(this, -1));
    this.container
      .querySelector("#moreBtn")
      ?.addEventListener("click", this.handleNumberButtonClick.bind(this, 1));
    // Number Input
  ...
  updateAbacus() {
    ...
    rowsDescending.forEach((rowGroup) => {
      ...
      const rowBeads = this.container
        .querySelector(`.row${rowGroup}`)
        ?.querySelectorAll("button.bead");
      if (!rowBeads) {
  ...
  updateNumberInput() {
    ...
    this.rows.forEach((rowNumber) => {
      const countedBeads = this.container
        .querySelector(`.row${rowNumber}`)
        ?.querySelectorAll("button.counted");
